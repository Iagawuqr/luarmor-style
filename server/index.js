...
app.use(async (req, res, next) => {
    ...
    // PERMITE TODAS AS ROTAS DA API ADMIN SEM BLOQUEIO
...
// ============ ADMIN AUTH CORRIGIDO ============
const adminAuth = (req, res, next) => {
    const k = req.headers['x-admin-key'] || req.query.key;
    
    console.log('=== ADMIN AUTH DEBUG ===');
    console.log('Received key:', k ? `"${k}" (${k.length} chars)` : 'NULL');
    console.log('Config ADMIN_KEY exists:', !!config.ADMIN_KEY);
    console.log('Config ADMIN_KEY length:', config.ADMIN_KEY ? config.ADMIN_KEY.length : 0);
    console.log('Config ADMIN_KEY preview:', config.ADMIN_KEY ? `"${config.ADMIN_KEY.substring(0, 5)}..."` : 'NULL');
    
    if (!k) {
        console.log('❌ Auth failed: No key provided');
    ...
    if (!config.ADMIN_KEY) {
        console.log('❌ Auth failed: Server misconfigured - ADMIN_KEY not set');
    ...
    // COMPARAÇÃO SIMPLES (sem secureCompare para debug)
    const keysMatch = k === config.ADMIN_KEY;
    console.log('Keys match (simple comparison)?', keysMatch);
    
    if (!keysMatch) {
        console.log('❌ Auth failed: Keys do not match');
        console.log('Expected:', config.ADMIN_KEY);
