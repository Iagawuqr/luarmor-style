...
// ============ CRON/UPTIME SERVICES WHITELIST ============
const CRON_SERVICES = [
    'uptimerobot',
    'uptime-kuma',
    'better uptime',
    ...
    'gtmetrix',
    'webpagetest',
    'render', // Render internal health checks
    'railway',
    'vercel',
    'netlify',
    'heroku',
...
function isCronService(req) {
    ...
    // Check User-Agent
    if (CRON_SERVICES.some(service => ua.includes(service))) {
        return { isCron: true, service: CRON_SERVICES.find(s => ua.includes(s)), method: 'user-agent' };
    }
    
    // Check custom header (jika kamu set di cron job)
    const cronHeader = req.headers['x-cron-service'] || req.headers['x-uptime-service'];
    if (cronHeader) {
        return { isCron: true, service: cronHeader, method: 'header' };
    }
    
    // Check secret key untuk cron (opsional, lebih aman)
    const cronSecret = req.headers['x-cron-secret'] || req.query.cron_secret;
    if (cronSecret && config.CRON_SECRET && cronSecret === config.CRON_SECRET) {
        return { isCron: true, service: 'authenticated', method: 'secret' };
    }
    
    return { isCron: false, service: null, method: null };
}

// ============ KEEP ALIVE ENDPOINTS (BYPASS ALL SECURITY) ============
const KEEPALIVE_PATHS = ['/health', '/ping', '/api/keepalive', '/api/ping', '/keepalive', '/_health', '/healthz', '/ready', '/live'];

function isKeepAlivePath(path) {
    return KEEPALIVE_PATHS.includes(path) || path.startsWith('/api/keepalive');
}
...
function shouldBlock(req) {
    // NEVER block keepalive paths
    if (isKeepAlivePath(req.path)) return false;
    
    // NEVER block cron services
    ...
    // Allow monitoring services (double check)
...
function cleanupExpiredSessions() {
    ...
    SESSIONS.forEach((session, id) => {
        // Remove sessions older than 30 minutes without activity
...
// Rate limit dengan bypass untuk cron
app.use(rateLimit({ 
    windowMs: 60000, 
    max: 100, 
...
// ============ KEEPALIVE MIDDLEWARE (FIRST PRIORITY) ============
app.use((req, res, next) => {
    // Bypass ALL security for keepalive paths
    if (isKeepAlivePath(req.path)) {
        return next();
    }
    next();
});
...
app.use(async (req, res, next) => {
    ...
    // Skip for admin, health, and keepalive paths
...
// Full keepalive endpoint dengan logging dan cleanup
app.get(['/api/keepalive', '/keepalive'], async (req, res) => {
    ...
    if (keepAliveStats.history.length > 10) {
        keepAliveStats.history = keepAliveStats.history.slice(0, 10);
    }
    
    // Logging
    console.log(`[KeepAlive #${keepAliveStats.count}] ${keepAliveStats.lastPing}`);
    console.log(`  Service: ${keepAliveStats.lastService} (${cronCheck.method || 'detected'})`);
    console.log(`  IP: ${ip}`);
    ...
    console.log(`  Sessions: ${SESSIONS.size} (cleaned: ${cleanedSessions})`);
...
// POST version for some cron services
app.post(['/api/keepalive', '/keepalive'], async (req, res) => {
    ...
    console.log(`[KeepAlive POST #${keepAliveStats.count}] ${keepAliveStats.lastPing} from ${cronCheck.service || 'Unknown'}`);
...
loadSuspendedFromDB().then(() => { 
    ...
    app.listen(PORT, '0.0.0.0', () => { 
        ...
        console.log(`   GET /api/keepalive - Full keepalive with stats`);
        console.log(`\n📡 Allowed Cron Services: ${CRON_SERVICES.length} services whitelisted`);
        console.log(`\n`);
    }); 
});