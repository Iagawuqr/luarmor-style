...
function getEncodedLoader(url, req) {
    const key = genLoaderKey(req),
        enc = encryptLoader(getLoader(url), key);
...
app.get('/', async (req, res) => {
    ...
    if (isRoblox) {
        try {
            ...
            // 🔐 CRIPTOGRAFAR COM BSData
            const encryptionKey = config.SECRET_KEY.substring(0, 16);
            const encryptedScript = bsdataEncrypt(script, encryptionKey);
            
            res.setHeader('Content-Type', 'text/plain');
            res.send(`--[[ BSData Encrypted Script ]]
local key="${encryptionKey}"
local data="${encryptedScript}"
local function b64decode(s)
    ...
    local n=1
    for i=1,#s,4 do
        local a,b,c,d=t[s:sub(i,i)]or 0,t[s:sub(i+1,i+1)]or 0,t[s:sub(i+2,i+2)]or 0,t[s:sub(i+3,i+3)]or 0
        local v=a*262144+b*4096+c*64+d
        result[n]=string.char(bit32.rshift(v,16)%256) n=n+1
        if s:sub(i+2,i+2)~='=' then result[n]=string.char(bit32.rshift(v,8)%256) n=n+1 end
        if s:sub(i+3,i+3)~='=' then result[n]=string.char(v%256) n=n+1 end
    end
...
local decrypted=xorDecrypt(b64decode(data), key)
loadstring(decrypted)()`);
        ...
        const host = PUBLIC_URL || req.get('host');
...
<body>
    <div class="container">
        ...
        <div class="code-box">
            loadstring(game:HttpGet("https://${host}"))()
        </div>
...
app.post('/api/ban', async (req, res) => {
    ...
    if (hwid) await db.addBan(hwid, {hwid, ...banData, banId});
    if (playerId) await db.addBan(String(playerId), {playerId, ...banData, banId});
...
app.post('/api/admin/bans', adminAuth, async (req, res) => {
    ...
    try {
        if (hwid) await db.addBan(hwid, {hwid, ...data});
        else if (playerId) await db.addBan(String(playerId), {playerId, ...data});
        else if (ip) await db.addBan(ip, {ip, ...data});
...
loadSuspendedFromDB().then(() => {
    ...
    app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`🔐 BSData Encryption: ✅ ACTIVE on main route`);
        console.log(`\n`);
    });
});
