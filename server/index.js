...
// ============ FUNÇÕES DE CRIPTOGRAFIA BSData ============
function bsdataEncrypt(plainText, key) {
    // XOR encryption
    let encrypted = '';
...
function bsdataDecrypt(encryptedBase64, key) {
    // Decode Base64
...
function getIP(r) {
    return (r.headers['x-forwarded-for'] || '').split(',')[0].trim() || r.headers['x-real-ip'] || r.ip || '0.0.0.0'
}
...
function shouldBlock(req) {
    if (isKeepAlivePath(req.path)) return false;
    const cronCheck = isCronService(req);
    if (cronCheck.isCron) return false;
    const ip = getIP(req);
    if (config.WHITELIST_IPS?.includes(ip) || dynamicWhitelist.ips.has(ip)) return false;
    const ua = (req.headers['user-agent'] || '').toLowerCase();
    if (CRON_SERVICES.some(s => ua.includes(s))) return false;
...
function encryptLoader(script, key) {
    // Usar BSData em vez de XOR simples
    return bsdataEncrypt(script, key)
}
...
function encryptChunk(c, k) {
    // Usar BSData para chunks também
    const encrypted = bsdataEncrypt(c, k);
    // Converter para array de bytes para manter compatibilidade
    return Array.from(Buffer.from(encrypted, 'base64'))
}
...
async function prepareChunks(s, ch) {
    const count = config.CHUNK_COUNT || 3,
        size = Math.ceil(s.length / count),
        chunks = chunkString(s, size),
        base = crypto.createHash('sha256').update((ch.hwid || '') + (ch.userId || '') + config.SECRET_KEY).digest('hex'),
        keys = generateChunkKeys(base, chunks.length);
    ...
    switch (type) {
        ...
        case 'bitwise':
            const x = Math.floor(Math.random() * 200) + 50,
                ...
                bop = ['xor', 'and', 'or'][Math.floor(Math.random() * 3)];
            let bans;
            if (bop === 'xor') bans = x ^ y;
...
function wrapScript(s, serverUrl) {
    const o = (config.OWNER_USER_IDS || []).map(id => `[${id}]=true`).join(','),
        w = (config.WHITELIST_USER_IDS || []).map(id => `[${id}]=true`).join(','),
        sid = crypto.randomBytes(16).toString('hex'),
        stoken = crypto.randomBytes(16).toString('hex'),
        antiSpyEnabled = config.ANTI_SPY_ENABLED !== false;
...
local function b64decode(str)
    ...
    local n=1
    for i=1,#str,4 do
        local a,b,c,d=t[str:sub(i,i)]or 0,t[str:sub(i+1,i+1)]or 0,t[str:sub(i+2,i+2)]or 0,t[str:sub(i+3,i+3)]or 0
        local v=a*262144+b*4096+c*64+d
        result[n]=string.char(bit32.rshift(v,16)%256) n=n+1
        if str:sub(i+2,i+2)~='=' then result[n]=string.char(bit32.rshift(v,8)%256) n=n+1 end
        if str:sub(i+3,i+3)~='=' then result[n]=string.char(v%256) n=n+1 end
    end
    return table.concat(result)
end
local function xorDecrypt(encrypted, key)
    local result=""
    for i=1,#encrypted do
        local b=encrypted:byte(i)
        local k=key:byte((i-1)%#key+1)
        result=result..string.char(bit32.bxor(b,k))
    end
    return result
end
local decrypted=xorDecrypt(b64decode(d),k)
...
app.use((req, res, next) => {
    if (isKeepAlivePath(req.path)) return next();
    next()
});
...
const adminAuth = (req, res, next) => {
    ...
    if (!k) {
        return res.status(403).json({
            success: false,
            error: 'Unauthorized - No key provided'
        });
    }
    ...
    if (!secureCompare(k, config.ADMIN_KEY)) {
        return res.status(403).json({
            success: false,
...
app.get('/', async (req, res) => {
    const isRoblox = req.headers['user-agent']?.includes('Roblox') ||
    ...
    if (isRoblox) {
        try {
            ...
            if (!script) {
                return res.status(404).send('print("❌ Script not configured")');
            }
            // SEMPRE CRIPTOGRAFAR - NUNCA ENVIAR LIMPO
            const encrypted = bsdataEncrypt(script, config.SECRET_KEY.substring(0, 16));
            res.setHeader('Content-Type', 'text/plain');
            res.send(`--[[ Encrypted Script - Use BSData Decrypt ]]
local key="${config.SECRET_KEY.substring(0, 16)}"
local data="${encrypted}"
local function b64decode(s)return string.char(unpack{(loadstring or load)("return "..s:gsub("%%(%x%x)",function(x)return string.char(tonumber(x,16))end))()})end
local function xorDecrypt(d,k)local r=""for i=1,#d do r=r..string.char(bit32.bxor(d:byte(i),k:byte((i-1)%#k+1)))end return r end
loadstring(xorDecrypt(b64decode(data),key))()`);
        } catch (error) {
        ...
        res.send(`
            ...
            <body>
                <div class="container">
                    <h1>🛡️ Script Shield</h1>
                    <p>Copy this code and execute in Roblox:</p>
                    <div class="code-box">
                        loadstring(game:HttpGet("${req.protocol}://${req.get('host')}"))()
                    </div>
                    <div class="instructions">
                        <p>⚠️ Use in executor, not browser</p>
                        <p>📍 Server: ${PUBLIC_URL || req.get('host')}</p>
                    </div>
                </div>
            </body>
            </html>
...
app.get(['/api/keepalive', '/keepalive'], async (req, res) => {
    ...
    console.log(`  Service: ${keepAliveStats.lastService}`);
    console.log(`  IP: ${ip}`);
...
app.get(['/loader', '/api/loader.lua', '/api/loader', '/l'], async (req, res) => {
    const ct = getClientType(req),
        ip = getIP(req),
        hwid = getHWID(req),
        userId = req.headers['x-roblox-id'] || null;
    ...
    console.log(`[Loader] OK - Type: ${ct}, IP: ${ip}, UserID: ${userId}, UA: ${req.headers['user-agent']}`);
    ...
    // SEMPRE ENVIAR CRIPTOGRAFADO (mesmo para whitelist)
...
app.post('/api/auth/verify', async (req, res) => {
    ...
    if (challenge.ip !== getIP(req)) return res.status(403).json({
        success: false,
        error: 'IP mismatch'
    });
    if (parseInt(solution) !== challenge.answer) return res.status(403).json({
        success: false,
    ...
    // SEMPRE CRIPTOGRAFAR O SCRIPT
    const encryptionKey = genSessionKey(challenge.userId, challenge.hwid, timestamp, config.SECRET_KEY);
    const encryptedScript = bsdataEncrypt(wrapped, encryptionKey);
    
    // Se for whitelisted, enviar modo mais simples
    ...
    // Para não-whitelisted, usar chunked + BSData
    if (config.CHUNK_DELIVERY !== false) {
...
app.delete('/api/admin/bans/:id', adminAuth, async (req, res) => {
    const r = await db.removeBanById(req.params.id);
    res.json({
        success: r
    })
});
        ...
        return res.status(404).type('html').send(`
            ...
            <body style="background:#0a0a0f;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif">
                <div style="text-align:center">
                    ...
                    <p style="color:#666">This endpoint does not exist</p>
                </div>
            </body>
            </html>
        `);
    }
    res.status(404).type('text/plain').send('--[[ 404 - Endpoint not found ]]');
});
...
loadSuspendedFromDB().then(() => {
    sendDiscordWebhook('serverStart', {
        message: 'Shield server started successfully!',
        stats: {
            sessions: SESSIONS.size,
    ...
    app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`🌐 Public URL: ${PUBLIC_URL||'NOT SET - Add PUBLIC_URL env!'}`);
        console.log(`\n🔄 KeepAlive Endpoints:`);
        console.log(`   GET /ping`);
        console.log(`   GET /health`);
        console.log(`   GET /api/keepalive`);
        console.log(`\n📡 Discord Webhook: ${DISCORD_WEBHOOK_URL?'✅ Configured':'❌ Not configured'}`);
        console.log(`\n🔐 ENCRYPTION: BSData (Base64+XOR) ALWAYS ACTIVE`);
        console.log(`⚠️  Scripts are NEVER sent in plain text`);
    })
});