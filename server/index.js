...
function isKeepAlivePath(p) {
    return KEEPALIVE_PATHS.includes(p) || p.startsWith('/api/keepalive');
}

function isCronService(req) {
    const ua = (req.headers['user-agent'] || '').toLowerCase();
    if (CRON_SERVICES.some(s => ua.includes(s))) return {
        isCron: true,
        service: CRON_SERVICES.find(s => ua.includes(s)),
        method: 'user-agent'
    };
    const ch = req.headers['x-cron-service'] || req.headers['x-uptime-service'];
    if (ch) return {
        isCron: true,
        service: ch,
        method: 'header'
    };
    const cs = req.headers['x-cron-secret'] || req.query.cron_secret;
    if (cs && config.CRON_SECRET && cs === config.CRON_SECRET) return {
        isCron: true,
...
function getClientType(req) {
    const ua = (req.headers['user-agent'] || '').toLowerCase();
    const h = req.headers;
    if (h['x-hwid'] || h['x-roblox-id'] || h['x-place-id']) return 'executor';
...
function genFakeScript() {
    ...
    const rH = l => {
        let h = '';
        for (let i = 0; i < l; i++) h += Math.floor(Math.random() * 16).toString(16);
        return h;
    };
...
async function sendDiscordWebhook(type, data) {
    ...
    try {
        ...
        if (data.userId) embed.fields.push({
            name: '👤 User ID',
            value: `\`${data.userId}\``,
            inline: true
        });
        if (data.hwid) embed.fields.push({
            name: '🔑 HWID',
            value: `\`${String(data.hwid).substring(0, 16)}...\``,
            inline: true
        });
        if (data.ip) embed.fields.push({
            name: '🌐 IP',
            value: `\`${data.ip}\``,
            inline: true
        });
        if (data.placeId) embed.fields.push({
            name: '🎯 Place ID',
            value: `\`${data.placeId}\``,
            inline: true
        });
        if (data.reason) embed.fields.push({
            name: '📝 Reason',
            value: `\`${data.reason}\``,
            inline: false
        });
        if (data.tool) embed.fields.push({
            name: '🔧 Tool',
            value: `\`${data.tool}\``,
            inline: true
        });
        if (data.action) embed.fields.push({
            name: '⚡ Action',
            value: `\`${data.action}\``,
            inline: true
        });
        if (data.executor) embed.fields.push({
            name: '💻 Executor',
            value: `\`${String(data.executor).substring(0, 50)}\``,
            inline: false
        });
        if (data.banId) embed.fields.push({
            name: '🆔 Ban ID',
            value: `\`${data.banId}\``,
            inline: true
        });
        if (data.bannedBy) embed.fields.push({
            name: '👮 Banned By',
            value: `\`${data.bannedBy}\``,
            inline: true
        });
        if (data.message) embed.description = data.message;
        if (data.stats) embed.fields.push({
            name: '📊 Stats',
            value: `Sessions: \`${data.stats.sessions || 0}\`\nUptime: \`${data.stats.uptime || '?'}\``,
            inline: false
        });
...
async function suspendUser(type, value, data) {
    const entry = {
        ...
        expiresAt: data.duration ? new Date(Date.now() + parseInt(data.duration) * 1000).toISOString() : null
    };
    ...
    sendDiscordWebhook('suspicious', {
        userId: data.userId,
        hwid: data.hwid,
        ip: data.ip,
        reason: 'Suspended: ' + (data.reason || 'Admin action'),
        action: 'Suspended'
    }).catch(() => {});
}
...
function checkSuspended(h, u, sid) {
    ...
    const check = (m, k) => {
        if (m.has(k)) {
            const s = m.get(k);
            if (!s.expiresAt || new Date(s.expiresAt).getTime() > now) return {
                suspended: true,
...
async function loadSuspendedFromDB() {
    ...
    if (all && all.length > 0) all.forEach(s => {
...
n("Loading","Connecting...",2) 
local c=hp(S.."/api/auth/challenge",{userId=L.UserId,hwid=(gethwid and gethwid() or "UNK"),placeId=game.PlaceId}) 
if c and c.success then 
    n("Loading","Verifying...",2) 
    local v=hp(S.."/api/auth/verify",{challengeId=c.challengeId,solution=sv(c),timestamp=os.time()}) 
    if v and v.success then 
        n("Loading","Loading...",2) 
        local fs=asm(v) 
        if fs then 
            local f,e=loadstring(fs) 
            if f then 
                pcall(f) 
                n("Success","Loaded!",2) 
            else 
                n("Error","Syntax: "..(e or "?"),5) 
            end 
        else 
            n("Error","Assembly Failed",5) 
        end 
    else 
        n("Error","Verify Failed: "..(v and v.error or "Unknown"),5) 
    end 
else 
    n("Error","Auth Failed: "..(c and c.error or "Connection"),5) 
end`;
}
...
function getEncodedLoader(url, req) {
    ...
    return `local k="${key}"
local d="${enc}"
local function x(s,k)local r={}local b={}for i=1,#s do b[i]=s:byte(i)end;for i=1,#b do r[i]=string.char(bit32.bxor(b[i],k:byte((i-1)%#k+1)))end;return table.concat(r)end;
local function b(s)local t={}local c="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"for i=1,64 do t[c:sub(i,i)]=i-1 end;s=s:gsub("[^"..c.."=]","")local r={}local n=1;for i=1,#s,4 do local a,b,c,d=t[s:sub(i,i)]or 0,t[s:sub(i+1,i+1)]or 0,t[s:sub(i+2,i+2)]or 0,t[s:sub(i+3,i+3)]or 0;local v=a*262144+b*4096+c*64+d;r[n]=string.char(bit32.rshift(v,16)%256)n=n+1;if s:sub(i+2,i+2)~="="then r[n]=string.char(bit32.rshift(v,8)%256)n=n+1 end;if s:sub(i+3,i+3)~="="then r[n]=string.char(v%256)n=n+1 end end;return table.concat(r)end;
loadstring(x(b(d),k))()`;
}
...
function cleanupExpiredSessions() {
    ...
    SESSIONS.forEach((session, id) => {
        if (now - session.lastSeen > 30 * 60 * 1000) {
            SESSIONS.delete(id);
            cleaned++;
        }
    });
...
const LOADER_HTML = fs.existsSync(path.join(viewsPath, 'loader/index.html')) ? fs.readFileSync(path.join(viewsPath, 'loader/index.html'), 'utf8') : `<!DOCTYPE html><html><head><title>Loader</title></head><body style="background:#0a0a0f;color:#fff;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif"><div style="text-align:center"><h1>🛡️ Shield Loader</h1><p style="color:#888">Execute via Roblox executor</p></div></body></html>`;
...
app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'x-admin-key', 'x-hwid', 'x-roblox-id', 'x-place-id', 'x-job-id', 'x-session-id', 'x-cron-service', 'x-cron-secret', 'x-uptime-service']
}));
...
app.use(rateLimit({
    ...
    skip: req => isKeepAlivePath(req.path) || isCronService(req).isCron
}));
...
// MIDDLEWARE DE VERIFICAÇÃO DE BANIMENTO
app.use(async (req, res, next) => {
    const adminPath = config.ADMIN_PATH || '/admin';
    if (req.path.startsWith(adminPath) || isKeepAlivePath(req.path)) return next();
    if (req.path === '/loader' || req.path === '/l' || req.path === '/api/loader' || req.path === '/api/loader.lua') return next();
    const cronCheck = isCronService(req);
    if (cronCheck.isCron) return next();
...
const adminAuth = (req, res, next) => {
    ...
    if (!k) return res.status(403).json({
        success: false,
        error: 'Unauthorized: No key'
    });
    if (!config.ADMIN_KEY) return res.status(500).json({
        success: false,
        error: 'Server misconfigured'
    });
    if (!secureCompare(k, config.ADMIN_KEY)) return res.status(403).json({
        success: false,
...
app.get('/api/admin/keys', adminAuth, async (req, res) => {
    try {
        const keys = await Keys.getAllKeys();
...
// Create new key
app.post('/api/admin/keys/create', adminAuth, async (req, res) => {
    try {
        const { note, days, hours, uses, hwidLock, ipLock, userIdLock } = req.body;
        const result = await Keys.createKey({
            note: note || '',
            days: parseInt(days) || 0,
            hours: parseInt(hours) || 0,
            uses: parseInt(uses) || 1,
            hwidLock: hwidLock === true || hwidLock === 'true',
            ipLock: ipLock === true || ipLock === 'true',
            userIdLock: userIdLock === true || userIdLock === 'true'
        });
        res.json(result);
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
...
app.delete('/api/admin/keys/:key', adminAuth, async (req, res) => {
    try {
        const deleted = await Keys.deleteKey(req.params.key);
...
// Disable key
app.post('/api/admin/keys/:key/disable', adminAuth, async (req, res) => {
    try {
        const disabled = await Keys.disableKey(req.params.key);
...
// Enable key
app.post('/api/admin/keys/:key/enable', adminAuth, async (req, res) => {
    try {
        const enabled = await Keys.enableKey(req.params.key);
...
// Get key stats
app.get('/api/admin/keys/:key/stats', adminAuth, async (req, res) => {
    try {
        const stats = await Keys.getKeyStats(req.params.key);
        if (stats) {
...
// Get usage stats overview
app.get('/api/admin/keys/stats/overview', adminAuth, async (req, res) => {
    try {
        const stats = await Keys.getUsageStats();
        res.json({ success: true, stats });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
...
app.get('/api/admin/keys/:key/info', adminAuth, async (req, res) => {
    try {
        const keyData = await Keys.getKey(req.params.key);
        if (keyData) {
...
// Toggle key status
app.post('/api/admin/keys/:key/toggle', adminAuth, async (req, res) => {
    try {
        const keyData = await Keys.getKey(req.params.key);
        if (!keyData) {
        ...
        const newStatus = !keyData.enabled;
        const result = newStatus ? 
            await Keys.enableKey(req.params.key) : 
            await Keys.disableKey(req.params.key);
            
        res.json({ 
            success: true, 
            enabled: newStatus,
            message: `Key ${newStatus ? 'enabled' : 'disabled'} successfully`
        });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
...
// Check key validity (public endpoint for loaders)
app.post('/api/verify-key', async (req, res) => {
    try {
        const { key, hwid, ip, userId } = req.body;
        
        if (!key) {
            return res.status(400).json({ success: false, error: 'Key required' });
        }
        
        const validation = await Keys.validateKey(key, hwid, ip, userId);
...
// Activate key (public endpoint for loaders)
app.post('/api/activate-key', async (req, res) => {
    try {
        ...
        const activation = await Keys.activateKey(key, hwid, ip, userId);
...
app.post('/api/admin/keys/check-usage', adminAuth, async (req, res) => {
    try {
        const { hwid, ip, userId } = req.body;
        const usedKeys = await Keys.checkExistingUsage(hwid, ip, userId);
        res.json({ success: true, usedKeys });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
});
...
// Clear expired keys
app.post('/api/admin/keys/clear-expired', adminAuth, async (req, res) => {
    try {
        const cleared = await Keys.clearExpiredKeys();
        res.json({ success: true, cleared, message: `Cleared ${cleared} expired keys` });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
});
...
app.get('/api/admin/keys/activations/recent', adminAuth, async (req, res) => {
    try {
        const stats = await Keys.getUsageStats();
        res.json({ success: true, activations: stats.recentActivations || [] });
    } catch (e) {
        res.status(500).json({ success: false, error: e.message });
    }
});
...
app.get('/health', (req, res) => {
    res.json({
        ...
        redis: db.isRedisConnected?.() ?? false
    });
});

app.get(['/_health', '/healthz', '/ready', '/live'], (req, res) => {
    res.json({
        status: 'ok',
        timestamp: new Date().toISOString()
    });
...
app.get(['/api/keepalive', '/keepalive'], async (req, res) => {
    const cronCheck = isCronService(req);
    const ip = getIP(req);
    keepAliveStats.count++;
    keepAliveStats.lastPing = new Date().toISOString();
    keepAliveStats.lastService = cronCheck.service || 'Unknown';
    keepAliveStats.history.unshift({
        time: keepAliveStats.lastPing,
        service: keepAliveStats.lastService,
        ip: ip,
        method: cronCheck.method
    });
    if (keepAliveStats.history.length > 10) keepAliveStats.history = keepAliveStats.history.slice(0, 10);
    console.log(`[KeepAlive #${keepAliveStats.count}] ${keepAliveStats.lastPing}`);
    ...
    res.json({
        ...
        timestamp: keepAliveStats.lastPing,
        stats: {
            pingCount: keepAliveStats.count,
            uptime: Math.floor(process.uptime()),
        ...
        cron: {
            detected: cronCheck.isCron,
            service: cronCheck.service,
            method: cronCheck.method
        },
        db: dbStatus,
        nextSleepIn: '~15 minutes without activity'
    });
});

app.post(['/api/keepalive', '/keepalive'], async (req, res) => {
    const cronCheck = isCronService(req);
    keepAliveStats.count++;
    keepAliveStats.lastPing = new Date().toISOString();
    keepAliveStats.lastService = cronCheck.service || 'POST request';
    console.log(`[KeepAlive POST #${keepAliveStats.count}] ${keepAliveStats.lastPing}`);
...
app.get(['/loader', '/api/loader.lua', '/api/loader', '/l'], async (req, res) => {
    ...
    // Para navegadores: mostrar HTML
    if (ct === 'browser') {
        console.log(`[Loader] BROWSER - IP: ${ip}`);
        await logAccess(req, 'LOADER_BROWSER', true, {
            clientType: ct,
            ip
        });
    ...
    // Para bots: sempre retornar script fake
    if (ct === 'bot') {
        console.log(`[Loader] BOT - IP: ${ip}, UA: ${req.headers['user-agent']}`);
        await logAccess(req, 'LOADER_BOT_FAKE', false, {
            clientType: ct,
            ip
        });
    ...
    // Para executores: verificar se tem headers necessários
    if (ct === 'executor') {
        console.log(`[Loader] EXECUTOR - Type: ${ct}, IP: ${ip}, UserID: ${userId}`);
        ...
        // Se não tem headers de executor, retorna fake
        if (!hwid && !userId && !req.headers['x-place-id']) {
            console.log(`[Loader] NO HEADERS - Returning fake script`);
            return res.status(200).type('text/plain').send(genFakeScript());
        }
        
        // Retorna loader codificado ou normal
        if (config.ENCODE_LOADER !== false && !isWL) {
            return res.type('text/plain').send(getEncodedLoader(url, req));
        } else {
            return res.type('text/plain').send(getLoader(url));
        }
    }
    
    // Para desconhecidos: retornar fake
    console.log(`[Loader] UNKNOWN - Type: ${ct}, IP: ${ip}`);
    return res.status(200).type('text/plain').send(genFakeScript());
});
...
app.post('/api/auth/challenge', async (req, res) => {
    const ct = getClientType(req);
    if (ct === 'bot') {
        await logAccess(req, 'CHALLENGE_BLOCKED', false, {
            clientType: ct
        });
    ...
    if (!userId || !placeId) return res.status(400).json({
        success: false,
        error: 'Missing userId or placeId'
    });
    if (config.REQUIRE_HWID && !hwid) return res.status(400).json({
        success: false,
        error: 'HWID required'
    });
    const uid = parseInt(userId),
        pid = parseInt(placeId);
    if (isNaN(uid) || isNaN(pid)) return res.status(400).json({
        success: false,
    ...
    if (susp) return res.json({
        success: false,
        error: 'Suspended: ' + susp.reason
    });
    if (!isWL) {
        const ban = await db.isBanned(hwid, ip, uid);
        if (ban.blocked) return res.json({
            success: false,
            error: 'Banned: ' + ban.reason
        });
    }
    if (config.ALLOWED_PLACE_IDS && config.ALLOWED_PLACE_IDS.length > 0 && !config.ALLOWED_PLACE_IDS.includes(pid) && !isWL) return res.status(403).json({
        success: false,
        error: 'Game not authorized'
    });
    await logAccess(req, 'CHALLENGE', true, {
        clientType: ct,
        whitelisted: isWL,
        userId: uid
    });
...
app.post('/api/auth/verify', async (req, res) => {
    const ct = getClientType(req);
    if (ct === 'bot') return res.status(403).json({
        success: false,
        error: 'Access denied'
    });
    const {
    ...
    if (!challengeId || solution === undefined || !timestamp) return res.status(400).json({
        success: false,
        error: 'Missing fields'
    });
    const challenge = await db.getChallenge(challengeId);
    if (!challenge) return res.status(403).json({
        success: false,
        error: 'Challenge expired'
    });
    const currentIP = getIP(req);
    if (challenge.ip !== currentIP && !challenge.whitelisted) {
        console.log(`[Verify] IP mismatch - Expected: ${challenge.ip}, Got: ${currentIP}`);
        await logAccess(req, 'VERIFY_IP_MISMATCH', false, {
    ...
    if (parseInt(solution) !== challenge.answer) {
        await logAccess(req, 'VERIFY_WRONG', false, {
            userId: challenge.userId
        });
        return res.status(403).json({
            success: false,
    ...
    if (!script) return res.status(500).json({
        success: false,
        error: 'Script not configured'
    });
    const url = getPublicUrl(req);
    ...
    sendDiscordWebhook('execution', {
        userId: challenge.userId,
        hwid: challenge.hwid,
        placeId: challenge.placeId,
        ip: currentIP,
        executor: req.headers['user-agent']
    }).catch(() => {});
    await logAccess(req, 'VERIFY_SUCCESS', true, {
        userId: challenge.userId
    });
...
app.post('/api/webhook/suspicious', async (req, res) => {
    ...
    await logAccess(req, 'SUSPICIOUS', false, {
        userId,
        hwid,
        tool,
        reason
...
app.post('/api/ban', async (req, res) => {
    ...
    if (!hwid && !playerId) return res.status(400).json({
        error: 'Missing id'
    });
    ...
    if (hwid) await db.addBan(hwid, {
        hwid,
        ...banData,
        banId
    });
    if (playerId) await db.addBan(String(playerId), {
        playerId,
        ...banData,
        banId
    });
    if (sessionId) SESSIONS.delete(sessionId);
    console.log(`[Ban] Banned - HWID: ${hwid}, PlayerID: ${playerId}, BanID: ${banId}`);
    await logAccess(req, 'BAN_ADDED', true, {
        hwid,
        playerId,
        reason,
        banId
    });
    sendDiscordWebhook('ban', {
        userId: playerId,
        hwid,
        ip: getIP(req),
        reason,
...
app.get('/api/admin/stats', adminAuth, async (req, res) => {
    try {
        ...
        const banCount = await db.getBanCount();
        const keyStats = await Keys.getUsageStats();
        res.json({
            success: true,
            stats: {
                ...s,
                ...
                activeKeys: keyStats.activeKeys || 0,
                expiredKeys: keyStats.expiredKeys || 0,
                totalActivations: keyStats.totalActivations || 0,
                sessions: SESSIONS.size
            },
            sessions: SESSIONS.size,
...
app.get('/api/admin/keepalive-stats', adminAuth, async (req, res) => {
    res.json({
        success: true,
        keepAlive: {
            ...keepAliveStats,
...
app.post('/api/admin/bans', adminAuth, async (req, res) => {
    ...
    if (!hwid && !ip && !playerId) return res.status(400).json({
        success: false,
        error: 'Required: hwid, ip, or playerId'
    });
    const banId = crypto.randomBytes(8).toString('hex').toUpperCase();
    const data = {
    ...
    try {
        ...
        else if (playerId) await db.addBan(String(playerId), {
            playerId,
            ...data
        });
        else if (ip) await db.addBan(ip, {
            ip,
            ...data
        });
        console.log(`[Admin] Banned - banId: ${banId}`);
        sendDiscordWebhook('ban', {
            userId: playerId,
            hwid,
            ip,
...
app.delete('/api/admin/bans/:id', adminAuth, async (req, res) => {
    try {
        ...
        console.log(`[Admin] Removing ban: ${banId}`);
        const r = await db.removeBanById(banId);
        if (r) {
            console.log(`[Admin] Ban removed: ${banId}`);
            res.json({
                success: true,
                message: 'Ban removed'
            });
        } else {
            console.log(`[Admin] Ban not found: ${banId}`);
            res.json({
                success: false,
                error: 'Ban not found'
            });
...
app.get('/api/admin/whitelist', adminAuth, async (req, res) => {
    res.json({
        ...
        whitelist: {
            ...
            ips: [...(config.WHITELIST_IPS || []), ...Array.from(dynamicWhitelist.ips)],
...
app.post('/api/admin/whitelist', adminAuth, async (req, res) => {
    ...
    else if (type === 'ip') dynamicWhitelist.ips.add(String(value));
...
app.post('/api/admin/whitelist/remove', adminAuth, async (req, res) => {
    ...
    else if (type === 'ip') dynamicWhitelist.ips.delete(String(value));
    res.json({
        success: true,
        msg: `Removed ${type}: ${value}`
    });
});

app.get('/api/admin/suspended', adminAuth, async (req, res) => {
    const a = [];
    suspendedUsers.hwids.forEach((v, k) => a.push({
        type: 'hwid',
        value: k,
        ...v
    }));
    suspendedUsers.userIds.forEach((v, k) => a.push({
        type: 'userId',
        value: k,
        ...v
    }));
    suspendedUsers.sessions.forEach((v, k) => a.push({
        type: 'session',
        value: k,
        ...v
    }));
...
app.post('/api/admin/suspend', adminAuth, async (req, res) => {
    ...
    if (!['hwid', 'userId', 'session'].includes(type)) return res.status(400).json({
        success: false,
        error: 'Invalid type'
    });
    const d = {
    ...
    if (type === 'hwid') suspendedUsers.hwids.set(String(value), d);
    else if (type === 'userId') suspendedUsers.userIds.set(String(value), d);
    else if (type === 'session') suspendedUsers.sessions.set(String(value), d);
    await db.addSuspend(type, String(value), d);
    res.json({
        success: true,
...
app.post('/api/admin/kill-session', adminAuth, async (req, res) => {
    ...
    await suspendUser('session', sessionId, {
        reason: reason || 'Killed by admin',
...
app.get('/api/admin/sessions', adminAuth, async (req, res) => {
    const arr = [];
    SESSIONS.forEach((v, k) => arr.push({
        sessionId: k,
        ...v,
        age: Math.floor((Date.now() - v.created) / 1000)
    }));
    res.json({
        success: true,
        sessions: arr.sort((a, b) => b.created - a.created)
    });
});

app.get('/api/admin/script', adminAuth, async (req, res) => {
    try {
        const script = await getScript();
        if (!script) return res.status(404).json({
            success: false,
        ...
        res.json({
            ...
            lines: script.split('\n').length,
            cached: !!(await db.getCachedScript())
        });
    } catch (e) {
        res.status(500).json({
            success: false,
...
app.use('*', (req, res) => {
    const ct = getClientType(req);
    if (ct === 'browser') return res.status(404).type('html').send(TRAP_HTML);
    res.status(403).type('text/plain').send(genFakeScript());
});

const PORT = process.env.PORT || config.PORT || 3000;
loadSuspendedFromDB().then(() => {
    sendDiscordWebhook('serverStart', {
        message: 'Shield server started!',
        stats: {
            sessions: SESSIONS.size,
    ...
    app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`🔴 Redis: ${db.isRedisConnected() ? '✅ Connected' : '❌ Using Memory'}`);
        console.log(`🔑 Keys System: ✅ Ready`);
        console.log(`\n`);
    });
});