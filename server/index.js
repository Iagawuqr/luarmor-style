...
async function loadSuspendedFromDB() {
    ...
    if (all && all.length > 0) {
        ...
        console.log(`✅ Loaded ${all.length} suspended entries`);
    }
}
...
app.post('/api/auth/challenge', async (req, res) => {
    ...
    if (shouldBlock(req)) {
        await logAccess(req, 'CHALLENGE_BLOCKED', false, { clientType: ct });
    ...
    if (!isWL) {
        ...
        if (ban.blocked) return res.json({ success: false, error: 'Banned: ' + ban.reason });
    }
    
    if (config.ALLOWED_PLACE_IDS && config.ALLOWED_PLACE_IDS.length > 0 && !config.ALLOWED_PLACE_IDS.includes(pid) && !isWL) {
